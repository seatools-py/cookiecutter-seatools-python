import os
from .common import mkdir, create_file, add_poetry_script, str_format, add_docker_compose_script
...
pids=$(pgrep -f '{{cookiecutter.project_name}}/venv/bin/django')
if [ -z "$pids" ]; then
  echo "无正在运行中的进程, 忽略"
else
  # 通过for循环遍历所有进程ID
...
pids=$(pgrep -f '{{cookiecutter.project_name}}-[A-Za-z0-9_\\\\-]*-py3.[0-9]+/bin/django')
if [ -z "$pids" ]; then
  echo "无正在运行中的进程, 忽略"
else
  # 通过for循环遍历所有进程ID
